...
func (r *dashboardRepository) GetAllStatsOptimized(ctx context.Context, tx *gorm.DB, teacherID *string, activeDays int) (*repositories.DashboardStatsData, error) {
	...
	// Build the teacher filter condition
	teacherFilter := ""
	args := []interface{}{}

	if teacherID != nil && *teacherID != "" {
		teacherFilter = "WHERE assessments.created_by = ?"
		args = append(args, *teacherID)
	}

	// Calculate active users date threshold
	activeUsersDate := time.Now().AddDate(0, 0, -activeDays)
	args = append(args, activeUsersDate)

	// Single query using CTEs to get all stats at once
	query := fmt.Sprintf(`
		WITH assessment_stats AS (
			...
			FROM assessments
			LEFT JOIN questions ON questions.created_by = COALESCE(?, assessments.created_by)
			LEFT JOIN question_banks ON question_banks.created_by = COALESCE(?, assessments.created_by)
			%s
		),
		attempt_stats AS (
			SELECT
				COUNT(*) as total_attempts,
				COUNT(DISTINCT CASE WHEN assessment_attempts.created_at >= ? THEN assessment_attempts.student_id END) as active_users,
	...
	// Adjust args based on whether teacherID is provided
	var queryArgs []interface{}
	if teacherID != nil && *teacherID != "" {
		queryArgs = []interface{}{*teacherID, *teacherID, activeUsersDate, *teacherID}
	} else {
		// When no teacherID, we still need placeholders for the LEFT JOINs
		queryArgs = []interface{}{nil, nil, activeUsersDate}
	}

	var stats repositories.DashboardStatsData
	err := db.WithContext(ctx).Raw(query, queryArgs...).Scan(&stats).Error
	if err != nil {
